...
const sharedCommandOptionsInflight = new Map<
  string,
...
function getCommandInflightKey(
  providerId: string,
  cwd: string,
...
function normalizeUsageSnapshot(
  usage: unknown,
): { usedTokens: number | null; maxTokens: number | null } | null {
  ...
  const lastTokenUsage =
    asRecord(usageRecord.last_token_usage) ??
    asRecord(usageRecord.lastTokenUsage);
  const usedTokens =
...
function parseModelVariant(option: ModelOption) {
  ...
  const parenthetical = label.match(
    /^(.*)\((thinking|low|medium|high|xhigh)\)\s*$/i,
  );
  if (parenthetical) {
    return {
  ...
  try {
    ...
    return parsed
      .map((entry) => {
        ...
        const priority =
          typeof entry?.priority === "string" ? entry.priority : null;
        return {
...
function parseTodoEntriesFromDetail(detail?: string | null) {
  ...
  try {
    ...
    const entries = parsed
      ...
      .filter((entry): entry is { content: string; status: string } =>
        Boolean(entry),
      );
...
function compactConversation(
  ...
  const summary = head
    ...
    .map(
      (msg) =>
        `${msg.role === "user" ? "User" : "Assistant"}: ${msg.content.replace(/\s+/g, " ").trim()}`,
    )
    .join("\n");
  ...
  return [
    { role: "user" as const, content: "Previous conversation summary:" },
    {
      ...
      content: summary.length > 2200 ? `${summary.slice(0, 2200)}...` : summary,
    },
    ...tail,
  ];
}
...
function getAttachmentPreviewSrc(attachment: PromptAttachment) {
  if (attachment.kind === "image") {
    const mimeType =
      attachment.mimeType || inferMimeTypeFromName(attachment.name);
...
export function AITab({
  ...
  const [activeMetaPopover, setActiveMetaPopover] = useState<
    "local" | "access" | "context" | null
  >(null);
  ...
  const [opencodeCliStatus, setOpencodeCliStatus] =
    useState<OpenCodeCliStatus | null>(null);
  const [installingOpenCodeMethodId, setInstallingOpenCodeMethodId] = useState<
    string | null
  >(null);
  const [opencodeInstallMessage, setOpencodeInstallMessage] = useState<
    string | null
  >(null);
  ...
  useEffect(() => {
    if (system.platform === "win32" && system.isWindows10) {
      return;
    }
  ...
  }, [refreshOpenCodeCliStatus, system.isWindows10, system.platform]);
  ...
  const isWindows10Unsupported =
    system.platform === "win32" && system.isWindows10;
  const hasOpenCodeProvider = Boolean(selectedProvider);
  const opencodeInstalled =
    !isWindows10Unsupported &&
    hasOpenCodeProvider &&
    Boolean(opencodeCliStatus?.installed);
  const checkingOpenCodeCli =
    !isWindows10Unsupported && opencodeCliStatus === null;
  ...
  const isLoadingOpenCodeMeta =
    ...
    ((isLoadingModelOptions && modelOptions.length === 0) ||
      (isLoadingModeOptions && modeOptions.length === 0));
  const openCodeProviderKey = useMemo(
    () =>
      extractModelMeta("OpenCode/loading", "opencode/loading", "opencode")
        .providerKey,
    [],
  );
  ...
  useEffect(() => {
    if (
      opencodeInstalled &&
      !wasOpencodeInstalledRef.current &&
      isActiveRef.current
    ) {
  ...
  const selectedModelLabel = useMemo(() => {
    const selectedOption = modelOptions.find(
      (item) => item.id === selectedModelValue,
    );
  ...
  const selectedModelMeta = useMemo(() => {
    return extractModelMeta(
      selectedModelLabel,
      selectedModelValue,
      selectedProvider?.id || tab.providerId || null,
    );
  }, [
    selectedModelLabel,
    selectedModelValue,
  ...
  const modeLabel = selectedModeOption
    ? displayModeName(selectedModeOption)
    : displayModeId(selectedModeId);
  ...
  const groupedModelFamilies = useMemo(() => {
    ...
    for (const option of modelOptions) {
      ...
      const searchable =
        `${split.group} ${split.name} ${variant.effort} ${option.id}`.toLowerCase();
    ...
    return Array.from(grouped.entries())
      .map(([group, familiesInGroup]) => ({
        group,
        families: [...familiesInGroup].sort((a, b) =>
  ...
  const selectedModelContextWindow = useMemo(() => {
    const direct = modelOptions.find(
      (item) => item.id === selectedModelValue,
    )?.contextWindow;
  ...
  const resolvedContextWindow = useMemo(() => {
    if (
      ...
      Number.isFinite(selectedModelContextWindow) &&
      selectedModelContextWindow > 0
    ) {
      return selectedModelContextWindow;
    }
    const maxFromUsage = selectedModelUsage?.maxTokens;
    if (
      typeof maxFromUsage === "number" &&
      Number.isFinite(maxFromUsage) &&
      maxFromUsage > 0
    ) {
      return maxFromUsage;
    }
  ...
  const isStreamingUi =
    loading || isAssistantFlushActive || isThoughtFlushActive;
  const slashInputState = useMemo(() => {
    ...
    const token = (
      whitespaceIndex >= 0
        ? commandSegment.slice(0, whitespaceIndex)
        : commandSegment
    ).trim();
    return {
      active: true,
  ...
  const slashCommandMenuOpen =
    slashInputState.active && slashCommandSuggestions.length > 0;
  ...
  const lastAssistantThoughtIndex = useMemo(() => {
    ...
    for (let index = tab.messages.length - 1; index >= 0; index -= 1) {
      ...
      if (
        message.role === "assistant" &&
        parseThoughtMessage(message.content) !== null
      ) {
        return index;
      }
    }
  ...
  const updateTab = useCallback(
    (updater: (current: AITabData) => AITabData) => {
  ...
  const appendThoughtDelta = useCallback(
    (delta: string) => {
      if (!delta) {
        return;
      }
  ...
  const enqueueThoughtDelta = useCallback(
    (delta: string) => {
      if (!delta) {
        return;
      }
  ...
  const markdownComponents = useMemo<Components>(
    () => ({
      p: ({ children, ...props }) => (
        <p
          className="my-1.5 whitespace-pre-wrap leading-7 text-[#d2d2d2]"
          {...props}
        >
          {children}
        </p>
      ...
      ul: ({ children, ...props }) => (
        <ul
          className="my-2 list-disc pl-6 space-y-1 marker:text-[#d0d0d0]"
          {...props}
        >
          {children}
        </ul>
      ...
      ol: ({ children, ...props }) => (
        <ol
          className="my-2 list-decimal pl-6 space-y-1 marker:text-[#d0d0d0]"
          {...props}
        >
          {children}
        </ol>
      ...
      table: ({ children, ...props }) => (
        <table
          className="my-3 w-full border-collapse overflow-x-auto block"
          {...props}
        >
          {children}
        </table>
      ...
      th: ({ children, ...props }) => (
        <th
          className="border border-white/20 px-3 py-1.5 text-left text-[12px] font-medium text-[#d0d0d0]"
          {...props}
        >
          {children}
        </th>
      ...
      td: ({ children, ...props }) => (
        <td
          className="border border-white/15 px-3 py-1.5 text-[13px] text-[#c0c0c0]"
          {...props}
        >
          {children}
        </td>
      ...
      code: ({ children, className, ...props }) => {
        ...
        if (isInline) {
          return (
            <code
              className="rounded-md bg-white/[0.08] px-1.5 py-0.5 font-mono text-[13px] text-[#e0e0e0]"
              {...props}
            >
              {children}
            </code>
      ...
      pre: ({ children, ...props }) => (
        <pre
          className="my-3 overflow-x-auto rounded-xl border border-white/10 bg-[#111111]/90 px-4 py-3 font-mono text-[13px] leading-6 text-[#d0d0d0]"
          {...props}
        >
          {children}
        </pre>
    ...
    const unsubscribe = window.argent.ai.onStreamEvent((payload) => {
      ...
      if (event.type === "text-delta") {
        ...
        if (
          ...
          deferredTextDeltaRef.current =
            (deferredTextDeltaRef.current || "") + (event.delta || "");
        } else {
          ...
          const messages = current.messages
            .filter(
              (msg) =>
                !(
                  msg.role === "assistant" &&
                  parseThoughtMessage(msg.content) !== null
                ),
            )
            .map((msg) => {
      ...
      if (event.type === "done") {
        ...
        if (selectedProvider?.id && event.reply?.id) {
          void window.argent.ai
            ...
            .then((modeState) => {
              const nextModes = Array.isArray(modeState?.modes)
                ? modeState.modes
                : [];
  ...
  useEffect(() => {
    ...
    const timer = window.setInterval(() => {
      ...
      const hasActiveTools = statuses.some((status) =>
        isToolActiveStatus(status),
      );
      if (hasActiveTools) {
        return;
      }
      ...
      if (staleRequestId) {
        void window.argent.ai
          .streamCancel({ requestId: staleRequestId })
          .catch(() => {
            // no-op
          });
      }
  ...
  useEffect(() => {
    async function loadModels() {
      ...
      const hasFreshCache =
        ...
        Date.now() - (sharedModelOptionsFetchedAt[providerId] ?? 0) <
          MODEL_CACHE_TTL_MS;
      ...
      if (hasFreshCache) {
        ...
        const hasCurrent =
          ...
          resolved.some((item) => item.id === currentModel);
        const nextModel = hasCurrent
          ? currentModel
          : resolved[0]?.id || selectedProvider.model;
      ...
      try {
        ...
        const modeId =
          typeof modeState?.currentModeId === "string"
            ...
            : null;
        const modeSessionId =
          typeof modeState?.sessionId === "string" ? modeState.sessionId : null;
  ...
  useEffect(() => {
    async function loadCommands() {
      ...
      const inflightKey = getCommandInflightKey(
        providerId,
        cwd,
        activeSessionId,
      );
  ...
  const applySlashCommand = useCallback(
    (name: string) => {
      updateTab((current) => {
        ...
        const remainder =
          whitespaceIndex >= 0
            ? commandSegment.slice(whitespaceIndex + 1).trimStart()
            : "";
  ...
  async function buildAttachmentsFromFiles(files: File[]) {
    ...
    for (const file of files) {
      ...
      const name =
        file.name ||
        (filePath ? filePath.split(/[/\\]/).at(-1) : "attachment") ||
        "attachment";
  ...
  async function handleSend() {
    ...
    const hasUserMessages = cleanMessages.some(
      (msg) => msg.role === "user" && msg.content.trim().length > 0,
    );
    const shouldRetitle =
      !hasUserMessages &&
      (current.title.trim().length === 0 || current.title === "AI Chat");
    const nextTitle = shouldRetitle
      ? summarizeTabTitleFromPrompt(input)
      : current.title;
    ...
    try {
      ...
      if (cancelRequestedRef.current) {
        ...
        try {
          await window.argent.ai.streamCancel({
        ...
        {isWindows10Unsupported ? (
          <div className="flex h-full min-h-[380px] flex-1 items-center justify-center">
            <div className="w-full max-w-[640px] text-center">
              <div className="mx-auto grid h-10 w-10 grid-cols-2 gap-1">
                <span className="rounded-[2px] bg-[#dcdcdc]" />
                <span className="rounded-[2px] bg-[#dcdcdc]" />
                <span className="rounded-[2px] bg-[#dcdcdc]" />
                <span className="rounded-[2px] bg-[#dcdcdc]" />
              </div>
              <h2 className="mb-0 mt-5 text-[28px] font-semibold tracking-tight text-white">
                Windows 10 not supported
              </h2>
              <p className="mx-auto mb-0 mt-3 max-w-[560px] text-[14px] leading-6 text-[#8e8e8e]">
                AI chat in Argent relies on the OpenCode CLI runtime. On Windows
                10 this setup is currently unsupported and can fail with IPC and
                runtime errors.
              </p>
              <div className="mx-auto mt-6 max-w-[560px] space-y-2 text-[13px] text-[#c8c8c8]">
                <div>Use Windows 11 for native AI tab support.</div>
                <div>Or run OpenCode from the Terminal Tab.</div>
              </div>
            </div>
          </div>
        ) : checkingOpenCodeCli ? (
          ...
          <div className="flex h-full min-h-[380px] flex-1 items-center justify-center">
            <div className="w-full max-w-[620px] text-center">
              <div className="mx-auto flex h-14 w-14 items-center justify-center text-[#d8d8d8]">
                <ProviderGlyph
                  providerKey={openCodeProviderKey}
              ...
              <p className="mx-auto mb-0 mt-3 max-w-[520px] text-[14px] leading-6 text-[#8e8e8e]">
                Argent uses the OpenCode CLI for AI chat. Install it once and
                we&apos;ll wire the rest up automatically.
              </p>
              {opencodeInstallMessage ? (
              ...
              <div className="mt-6 flex flex-wrap items-center justify-center gap-3">
                {(opencodeCliStatus?.installMethods ?? []).map(
                  (method, index) => {
                    const isInstalling =
                      installingOpenCodeMethodId === method.id;
                    ...
                    return (
                      ...
                      >
                        ...
                        <span>
                          {isInstalling ? "Installing..." : method.label}
                        </span>
                      </button>
                    );
                  },
              ...
              opencodeCliStatus.installMethods.length === 0 ? (
                <div className="mx-auto mt-6 max-w-[560px] rounded-2xl border border-white/8 bg-white/[0.03] px-4 py-4 text-left">
                  <div className="text-[12px] font-medium uppercase tracking-[0.18em] text-[#767676]">
                    Manual Install
                  </div>
              ...
              (() => {
                ...
                if (parsedTool && isToolActiveStatus(parsedTool.status)) {
                  for (
                    ...
                    if (nextTool) {
                      const nextToolStatus = normalizeToolDisplayStatus(
                        nextTool.status,
                        loading,
                      );
                      if (
                        !isToolActiveStatus(nextToolStatus) ||
                        nextTool.kind !== parsedTool.kind ||
                        nextTool.title !== parsedTool.title
                      ) {
                        hasLaterVisibleMessage = true;
                        break;
                      }
                ...
                const toolData = parsedTool
                  ? {
                      ...parsedTool,
                      status: resolveRenderedToolStatus(
                        parsedTool.status,
                        loading,
                ...
                if (toolData) {
                  const isCompacting =
                    toolData.kind === "session" &&
                    toolData.title.trim().toLowerCase() === "compacting...";
                  const isTodoTool = /\btodos?\b/i.test(toolData.title);
                  const todoEntries = parseTodoEntriesFromDetail(
                    toolData.detail,
                  );
                  return (
                    <div
                      key={index}
                      className="mr-auto w-full max-w-full py-0.5"
                    >
                      <div className="flex max-w-full items-start gap-2.5 py-1 text-[13px]">
                        ...
                        <div className="min-w-0 flex-1">
                          <div
                            className={
                              isCompacting
                                ? "ai-compacting-shimmer truncate font-medium"
                                : "truncate text-[#cccccc]"
                            }
                          >
                            {toolData.title}
                          </div>
                          {todoEntries ? (
                            <div className="mt-1 space-y-1 text-[12px] text-[#9a9a9a]">
                              {todoEntries.map((entry, todoIndex) => (
                                <div
                                  key={`${entry.content}-${todoIndex}`}
                                  className="flex items-start justify-between gap-2"
                                >
                                  <div className="min-w-0 truncate text-[#bdbdbd]">
                                    {entry.content}
                                  </div>
                                  <div className="shrink-0 text-[11px] text-[#8d8d8d]">
                            ...
                            <div className="truncate text-[11px] text-[#7d7d7d]">
                              {toolData.detail}
                            </div>
                          ) : null}
                        </div>
                ...
                if (parsedThought !== null) {
                  ...
                  return (
                    <div
                      key={index}
                      className={`mr-auto w-full max-w-full py-1 ${isStreamingThought ? "" : "ai-message-enter"}`}
                    >
                      <blockquote
                        className={`ai-thinking-block m-0 border-l-2 border-white/15 bg-white/[0.04] px-3 py-2 text-[13px] italic text-[#8f8f8f] ${isStreamingThought ? "ai-thinking-active" : ""}`}
                      >
                        {isStreamingThought ? (
                    ...
                    {message.role === "assistant" ? (
                      <div>
                        <div className="prose prose-invert max-w-none prose-p:my-2 prose-p:leading-7 prose-headings:my-2 prose-strong:text-[#efefef] prose-em:text-[#d6d6d6] prose-code:text-[#d9d9d9] prose-pre:bg-[#111111]/90 prose-pre:border prose-pre:border-white/10 prose-pre:rounded-xl prose-blockquote:border-l-white/25 prose-blockquote:text-[#c9c9c9] prose-table:my-3 prose-table:w-full prose-th:border prose-th:border-white/20 prose-th:px-2 prose-th:py-1 prose-td:border prose-td:border-white/15 prose-td:px-2 prose-td:py-1 prose-hr:border-white/15">
                          ...
                          index === activeAssistantMessageIndexRef.current &&
                          (message.content || "").trim().length === 0 ? (
                            <span className="inline-block animate-pulse text-[#f0f0f0]">
                              ▌
                            </span>
                          ) : null}
                        </div>
                          ...
                          return (
                            <div className="mt-1.5 flex items-center justify-between text-[11px] text-[#8a8a8a] opacity-0 transition-opacity group-hover:opacity-100 group-focus-within:opacity-100">
                              ...
                              <span className="inline-flex max-w-full items-center gap-1.5 truncate pl-3 text-[#7d7d7d]">
                                <ProviderGlyph
                                  providerKey={assistantModelLabel.providerKey}
                                  className="h-3.5 w-3.5 text-[#8f8f8f]"
                                />
                                <span className="truncate">
                                  {assistantModelLabel.modelName}
                                </span>
                              </span>
                            </div>
                          );
                          ...
                          <div className="flex flex-wrap justify-end gap-2">
                            {userImageAttachments.map((attachment) => {
                              ...
                              return (
                                <div
                                  key={attachment.id}
                                  className="h-24 w-24 overflow-hidden rounded-2xl border border-white/10 bg-[#1d1d1d]"
                                >
                                  <img
                                    src={previewSrc}
                                    alt={attachment.name}
        ...
        >
          ...
          >
            <div className="px-2.5">
              {isComposerDragging ? (
                <div className="px-1 pt-2 text-[11px] text-[#9ac1ff]">
                  ...
                  {composerAttachments.map((attachment) => {
                    ...
                    if (previewSrc) {
                      return (
                        <div
                          key={attachment.id}
                          className="relative h-20 w-20 overflow-hidden rounded-2xl border border-[#3a3a3a] bg-[#1a1a1a]"
                        >
                          <img
                            src={previewSrc}
                            alt={attachment.name}
                    ...
                    return (
                      <div
                        key={attachment.id}
                        className="inline-flex max-w-full items-center gap-1.5 rounded-2xl border border-[#3a3a3a] bg-[#1a1a1a] px-1.5 py-1 text-[11px] text-[#d0d0d0]"
                      >
              ...
              <textarea
                ...
                onKeyDown={(event) => {
                  if (
                    ...
                    if (event.key === "ArrowDown") {
                      event.preventDefault();
                      setCommandMenuIndex(
                        (prev) => (prev + 1) % slashCommandSuggestions.length,
                      );
                      return;
                    }
                    ...
                    if (event.key === "ArrowUp") {
                      ...
                      setCommandMenuIndex(
                        (prev) =>
                          (prev - 1 + slashCommandSuggestions.length) %
                    ...
                    if (
                      ...
                      event.preventDefault();
                      const selected =
                        slashCommandSuggestions[commandMenuIndex] ??
                  ...
                  if (
                    !loading &&
                    opencodeInstalled &&
                    (tab.input.trim().length > 0 ||
                      (tab.attachments?.length ?? 0) > 0)
                  ) {
                    void handleSend();
                  }
                }}
                placeholder={
                  opencodeInstalled
                    ? "Ask Argent anything, @ to add files, / for commands"
                    : "Install OpenCode CLI to enable AI chat"
                }
                rows={3}
              />
                  ...
                  {slashCommandSuggestions.map((command, index) => {
                    ...
                    return (
                      ...
                      >
                        ...
                        {command.description ? (
                          <div className="mt-0.5 truncate text-[11px] text-[#8d8d8d]">
                            {command.description}
                          </div>
                        ) : null}
                      </button>
                ...
                <div className="relative flex items-center" ref={modelMenuRef}>
                  {isLoadingOpenCodeMeta ? (
                    <div className="inline-flex items-center gap-1.5 px-2 py-1.5 text-sm text-[#878787]">
                      ...
                      />
                      <span className="ai-compacting-shimmer tracking-[0.02em]">
                        Loading OpenCode
                      </span>
                    </div>
                  ) : (
                    <>
                      ...
                      >
                        <span className="inline-flex items-center gap-1.5 whitespace-nowrap">
                          <ProviderGlyph
                            providerKey={selectedModelMeta.providerKey}
                            className="h-3.5 w-3.5 text-[#bdbdbd]"
                          />
                      ...
                      <div className="relative" ref={modeMenuRef}>
                        ...
                        {activeMetaPopover === "access" ? (
                          <div
                            className="absolute left-0 bottom-[calc(100%+8px)] w-[280px] max-w-[70vw] rounded-xl border border-[#363636] bg-[#181818] p-1.5 shadow-[0_16px_40px_rgba(0,0,0,0.45)] z-40"
                            role="listbox"
                          >
                            <div className="grid gap-1">
                              {modeOptions.length > 0 ? (
                                modeOptions.map((mode) => {
                                  ...
                                  return (
                                    ...
                                    >
                                      ...
                                      {mode.description ? (
                                        <div className="mt-0.5 text-[11px] text-[#898989]">
                                          {mode.description}
                                        </div>
                                      ) : null}
                                    </button>
                                ...
                                <div className="px-2.5 py-2 text-[12px] text-[#8f8f8f]">
                                  Modes unavailable
                                </div>
                              )}
                            </div>
                  ...
                  <button
                    type="button"
                    className="bg-transparent hover:bg-white/8 text-[#b8b8b8] transition rounded-full p-1.5 outline-none"
                  ...
                  {modelMenuOpen ? (
                    <div
                      className="absolute left-0 bottom-[calc(100%+8px)] w-[440px] max-w-[80vw] rounded-xl border border-[#363636] bg-[#181818] p-1.5 shadow-[0_16px_40px_rgba(0,0,0,0.45)] z-40"
                      role="listbox"
                    >
                      <div className="p-1.5">
                        <input
                ...
                {!isLoadingOpenCodeMeta
                  ? (() => {
                      ...
                      const percentage = Math.min(
                        100,
                        Math.max(0, (usedTokens / limit) * 100),
                      );
                      ...
                      return (
                        <div className="relative">
                          ...
                          >
                            ...
                            >
                              ...
                              <circle
                                ...
                                strokeDashoffset={
                                  validLimit ? strokeDashoffset : circumference
                                }
                                strokeLinecap="round"
                              />
                            </svg>
                          ...
                          {activeMetaPopover === "context" ? (
                            <div
                              className="absolute right-0 bottom-[calc(100%+8px)] w-[250px] rounded-xl border border-[#353535] bg-[#161616] px-3.5 py-3 text-[12px] leading-relaxed shadow-[0_14px_30px_rgba(0,0,0,0.45)] z-40 text-left"
                              role="status"
                            >
                              <div className="font-medium text-[#c0c0c0] mb-0.5">
                                Context window
                              </div>
                              {validLimit ? (
                                <>
                                  <div className="font-medium text-[#e0e0e0] text-[13px] mb-1">
                                    ...
                                    <span className="text-[#888] font-normal">
                                      ({(100 - percentage).toFixed(1)}% left)
                                    </span>
                                  </div>
                                  <div className="text-[#888]">
                                    {(usedTokens / 1000).toFixed(1)}k /{" "}
                                    {(limit / 1000).toFixed(1)}k tokens used
                                  </div>
                                </>
                              ) : (
                                <div className="text-[#888]">
                                  Token limit is unavailable for the current
                                  model.
                                </div>
                              )}
                            </div>
